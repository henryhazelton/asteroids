...
class Asteroid(CircleShape):
    ...
    def draw(self, screen):
        ...
        )  # Hopefully this draws a circle correctly
    ...
    def split(self):
        ...
        random_angle = random.uniform(
            20, 50
        )  # Create a random angle between 20 & 50 degrees that the new asteroids will split at
        ...
        asteroid_1 = Asteroid(
            self.position.x, self.position.y, radius=new_radius_of_asteroid
        )
        asteroid_2 = Asteroid(
            self.position.x, self.position.y, radius=new_radius_of_asteroid
        )
